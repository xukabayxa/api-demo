...
namespace Modules\Motels\Http\Controllers\Api\Admin;
...
use Modules\Motels\Repositories\Criteria\MotelRequestCriteria;
...
class MotelsController extends BaseApiController
{
    ...
    {
        $motels = $this->repository->with(['comments','comments.creatable'])
            ->pushCriteria(new MotelRequestCriteria($request))
    ...
    public function destroy($id, DeleteMotelRequest $request)
    {
        ...
        try {
            ...
            DB::commit();
            return $this->responseSuccess('delete motel success.');
        } catch (\Exception $e) {
            Log::error($e);
            DB::rollBack();
            return $this->responseErrors(500, $e->getMessage());
